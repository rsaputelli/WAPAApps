    ...
    try:
        if name.endswith(".csv"):
            ...
            if sheet_hint and sheet_hint in xls.sheet_names:
                df = pd.read_excel(xls, sheet_name=sheet_hint); return df, sheet_hint
            for sh in xls.sheet_names:
                df = pd.read_excel(xls, sheet_name=sh)
                if ID_COL in df.columns or RAW_MEMBER_TYPE_COL in df.columns:
                    return df, sh
            sh = xls.sheet_names[0]; df = pd.read_excel(xls, sheet_name=sh)
            return df, sh
    finally:
...
with tab2:
    ...
    if f_left and f_right:
        ...
        def to_totals_from_members(df0):
            if RAW_MEMBER_TYPE_COL not in df0.columns: return pd.DataFrame(columns=["Member Type","count"])
        ...
        if has_ids_left and has_ids_right:
            ...
            # Summary-level compare (or legacy summaries)
            left_label = _extract_date_label(f_left, "Left")
            right_label = _extract_date_label(f_right, "Right")
            if is_legacy_totals(df_left_raw): totals_left = to_totals_from_legacy(df_left_raw)
            else: totals_left = to_totals_from_members(df_left)
            if is_legacy_totals(df_right_raw): totals_right = to_totals_from_legacy(df_right_raw)
            else: totals_right = to_totals_from_members(df_right)
        ...
        for f in files:
            ...
            try:
                ...
                if "Member Type Totals" not in xls.sheet_names:
                    st.error(f"{getattr(f,'name','file')}: missing required sheet 'Member Type Totals'."); continue
                df_tot = pd.read_excel(xls, sheet_name="Member Type Totals")
            except Exception as e:
                st.error(f"Failed to read {getattr(f,'name','file')}: {e}"); continue
            cols = set(df_tot.columns)
            if {"index","Member/Non-Member - Member Type"}.issubset(cols):
                df_t = df_tot.rename(columns={"index":"Member Type","Member/Non-Member - Member Type":"count"})[["Member Type","count"]].copy()
            elif {"Member Type","count"}.issubset(cols):
                df_t = df_tot[["Member Type","count"]].copy()
            else:
                st.error(f"{getattr(f,'name','file')}: missing required columns in 'Member Type Totals'."); continue
            df_t["Date"] = label; rows.append(df_t)
        if rows:
            ...
            line = alt.Chart(df_line).mark_line(point=True).encode(
                x="Date:N", y="Count:Q", color="Member Type:N", tooltip=["Date","Member Type","Count"]
            ...
            wb = Workbook(); ws = wb.active; ws.title = "Monthly Totals"
            ws.append(["Date"] + list(pivot.columns))
            for idx, row in pivot.iterrows(): ws.append([idx] + list(map(int, row.values)))
